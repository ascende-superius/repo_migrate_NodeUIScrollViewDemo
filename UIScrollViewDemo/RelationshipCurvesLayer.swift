...
class RelationshipCurvesLayer: CAShapeLayer
{
    ...
    final func redrawRelationshipCurves()
    {
        ...
        for targetNode in NodesPM.nodes.filter({$0 != NodesPM.resizingNode})
        {
            ...
            relationshipCurvesPath.append(rectPath)
            ...
            // draw relationships...
            for (idx, candidateNode) in targetNode.inputNodes.enumerated()
            {
                if let inputNode = candidateNode
                {
                    ...
                    relationshipCurvesPath.move(to: targetPosition)
                    relationshipCurvesPath.addCurve(to: inputPosition, controlPoint1: controlPointOne, controlPoint2: controlPointTwo)
                }
            }
        }
    ...
    func drawSemiCircle(relationshipCurvesPath: UIBezierPath, position: CGPoint, clockwise: Bool)
    {
        ...
        relationshipCurvesPath.move(to: position)
        
        for i in 1...3
        {
            relationshipCurvesPath.addArc(withCenter: position, radius: CGFloat(i * 2), startAngle: quarterCircle, endAngle: halfCircle + quarterCircle, clockwise: clockwise)
        }
    }
