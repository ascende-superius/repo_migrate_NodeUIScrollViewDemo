...
struct NodesPM
{
    ...
    static var selectedNode: NodeVO? = nil
    {
            ...
            if relationshipCreationMode
            {
                if let targetNode = newValue
                {
                    if let inputNode = selectedNode
                    {
                        ...
                        nodeUpdate(node: targetNode)
                        postNotification(notificationType: .RelationshipsChanged, payload: nil)
                    }
                }
            }
        ...
        didSet
        {
            postNotification(notificationType: .NodeSelected, payload: selectedNode)
    ...
    static func changeSelectedNodeOperator(newOperator: NodeOperators)
    {
        if let node = selectedNode
        {
            ...
            postNotification(notificationType: .NodeUpdated, payload: node)
            postNotification(notificationType: .RelationshipsChanged, payload: nil)
    ...
    static func changeSelectedNodeType(newType: NodeTypes)
    {
        if let node = selectedNode
        {
            ...
            else if node.nodeType == NodeTypes.Number
            {
                ...
                {
                    node.inputNodes = [NodeVO?](repeating: nil,count: node.maxInputNodeCount, )
                }
            }
    ...
    static func nodeUpdate(node: NodeVO, isRecursive: Bool = false)
    {
        ...
        for candidateNode in nodes.filter({!($0 == NodesPM.selectedNode!)})
        {
            ...
            for inputNode in candidateNode.inputNodes.filter({($0 == node)})
            {
                if inputNode == node && candidateNode.nodeType == NodeTypes.Operator
                {
                    ...
                    nodeUpdate(node: candidateNode, isRecursive: true)
                }
            }
        }
        ...
        if !isRecursive
        {
            ...
            {
                postNotification(notificationType: .NodeUpdated, payload: updatedNode)
            }
        }
    }
    ...
    static var isDragging: Bool = false
    {
        didSet
        {
            let _isDragging = NSNumber(value: isDragging)
            postNotification(notificationType: .DraggingChanged, payload: _isDragging)
        }
    }
    ...
    static var relationshipCreationMode: Bool = false
    {
        didSet
        {
            ...
            {
                let _relationshipCreationMode = NSNumber(value: relationshipCreationMode)
                postNotification(notificationType: .RelationshipCreationModeChanged, payload: _relationshipCreationMode)
            }
        }
    }
    ...
    {
        notificationCentre.addObserver(observer, selector: selector, name: Notification.Name( notificationType.rawValue), object: nil)
    }
    ...
    static func deleteSelectedNode()
    {
        ...
        for node in nodes
        {
            for (i, inputNode) in node.inputNodes.enumerated()
            {
        ...
        postNotification(notificationType: .NodeDeleted, payload: selectedNode)
        postNotification(notificationType: .RelationshipsChanged, payload: nil)
    ...
    static func createNewNode(origin: CGPoint)
    {
        ...
        postNotification(notificationType: .NodeCreated, payload: newNode)
    ...
    static func moveSelectedNode(position: CGPoint)
    {
        ...
        postNotification(notificationType: .NodesMoved, payload: nil)
    }
    
    private static func postNotification(notificationType: NodeNotificationTypes, payload: AnyObject?)
    {
        let notification = Notification(name: Notification.Name(rawValue: notificationType.rawValue), object: payload)
...
struct NodeConstants
{
    ...
    static let curveColor = UIColor.blue
    
    static let selectedNodeColor = UIColor.blue
