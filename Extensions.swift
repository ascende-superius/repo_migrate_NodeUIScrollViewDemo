...
extension UIColor
{
    ...
    {
        return UIColor(red: CGFloat(truncating: redComponent), green: CGFloat(truncating: greenComponent), blue: CGFloat(truncating: blueComponent), alpha: 1.0)
    }
    ...
    func getRGB() -> (redComponent: Float, greenComponent: Float, blueComponent: Float)
    {
        if self.cgColor.numberOfComponents == 4
        {
            ...
            let redComponent = zeroIfDodgy(value: Float(colorRef![0]))
            let greenComponent = zeroIfDodgy(value: Float(colorRef![1]))
            let blueComponent = zeroIfDodgy(value: Float(colorRef![2]))
